...
# steps
divide_matrices_custom <- function(data, fixed, random = NULL, analysis_type,
  ...
  if (!ord_cov_dummy){
    cont_var = all_vars(remove_lhs(fixed))
    xrefs = refs
    for (i in cont_var) {
      xrefs[[i]] = NULL
  ...
  if (!ord_cov_dummy){
    XXnam <- get_linpreds(fixed, random, data=as.data.frame(MX), models, auxvars, analysis_type,
                          warn = warn, refs = xrefs)
  } else {
